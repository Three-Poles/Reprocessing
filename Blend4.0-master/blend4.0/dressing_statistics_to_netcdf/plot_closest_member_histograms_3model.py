...
# --- read the previously generated data (dressing_statistics_to_netcdf.py)
...
nc.close()
print 'precip thresholds = ', precip_thresholds
...
for i in range(3):
    ...
    a1.plot(np.real(range(1,501))/500., closest_histogram_NCEP[i,:],'-',\
        color='Red',label=r'$\leftarrow \ $'+ctext_low_N+' NCEP '+\
        ctext_high_N+r'$\ \rightarrow$',linewidth=2)
    ...
    a1.plot(np.real(range(1,501))/500., closest_histogram_CMC[i,:],'-',\
        color='LimeGreen',label=r'$\leftarrow \ $'+ctext_low_C+' CMC '+\
        ctext_high_C+r'$\ \rightarrow$',linewidth=2)
    ...
    a1.plot(np.real(range(1,1251))/1250., closest_histogram_ECMWF[i,:],'-',\
        color='RoyalBlue',label=r'$\leftarrow \ $'+ctext_low_E+' ECMWF '+\
        ctext_high_E+r'$\ \rightarrow$',linewidth=2)
