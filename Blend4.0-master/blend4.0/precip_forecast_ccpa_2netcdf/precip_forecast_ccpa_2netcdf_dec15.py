...
for idate, date in zip(range(ndates), date_list):
    ...
    outfile = '/Users/thamill/precip/ecmwf_data/'+\
        center+'_'+date+'_leadtime'+cleade+'h.nc'
    print 'writing to ',outfile, timey.asctime()
    ...
    for imem in range(nmembers):
        ...
        cmem = str(imem+1)
        infile = '/Users/thamill/precip/ecmwf_data/'+center+'/'+ \
        ...
        if istat != -1:
            ...
            print 'file not available: ',infile
            fcst = -99.99*np.ones((nyf,nxf), dtype=np.float32)
