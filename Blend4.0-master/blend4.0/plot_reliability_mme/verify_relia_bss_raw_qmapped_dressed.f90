!  f2py -c -m verify_relia_bss_raw_qmapped_dressed verify_relia_bss_raw_qmapped_dressed.f90 read_prob_forecasts_raw_qmapped_dressed.f90 read_prob_forecasts_raw_qmapped_dressed_mme.f90 ran3.f sort.f check.f90 -I/usr/local/gfortran/include -L/usr/local/gfortran/lib  -L/opt/local/lib -lnetcdff

SUBROUTINE verify_relia_bss_raw_qmapped_dressed(cleade, cmodel, cftype, nclasses, rthresh, &
    date_list_anal, apcp_anal_t, validdays, nxa, nya, ndates, &
    relia_raw, relia_raw_05, relia_raw_95, frequse_raw, bss_raw, bs_raw_daily, &
    relia_qmapped, relia_qmapped_05, relia_qmapped_95, frequse_qmapped, bss_qmapped, bs_qmapped_daily, &
    relia_dressed, relia_dressed_05, relia_dressed_95, frequse_dressed, bss_dressed, bs_dressed_daily, &
    bs_climo_daily)
...
REAL, DIMENSION(nclasses), INTENT(OUT) :: frequse_raw, frequse_qmapped, frequse_dressed 
    
REAL, INTENT(OUT) :: bss_raw, bss_qmapped, bss_dressed 
REAL*8, INTENT(OUT), DIMENSION(ndates) :: bs_raw_daily
REAL*8, INTENT(OUT), DIMENSION(ndates) :: bs_qmapped_daily
REAL*8, INTENT(OUT), DIMENSION(ndates) :: bs_dressed_daily
REAL*8, INTENT(OUT), DIMENSION(ndates) :: bs_climo_daily

! f2py intent(in) cleade, cmodel, cftype, nclasses, rthresh, date_list_anal, apcp_anal_t
! f2py intent(in) nxa, nya, ndates  
! f2py intent(in,out) validdays

! f2py depend(ndates) date_list_anal
! f2py depend(nxa,nya,ndates) apcp_anal_t, validdays

! f2py intent(out) relia_raw, relia_raw_05, relia_raw_95, frequse_raw, bss_raw, bs_raw_daily, bs_climo_daily
! f2py intent(out) relia_qmapped, relia_qmapped_05, relia_qmapped_95, frequse_qmapped, bss_qmapped, bs_qmapped_daily
! f2py intent(out) relia_dressed, relia_dressed_05, relia_dressed_95, frequse_dressed, bss_dressed, bs_dressed_daily
...
! f2py depend(nclasses) frequse_raw, frequse_qmapped, frequse_dressed
! f2py depend(nthreshes) bss_raw, bss_qmapped, bss_dressed
! f2py depend(ndates) bs_raw_daily, bs_qmapped_daily, bs_dressed_daily, bs_climo_daily
...
DO itype = 1, 3
	...
	DO iresa = 1, nresa
		...
		DO idate = 1, ndates
            ...
            IF (bs_daily(idate2) .gt. 0.0) THEN
...
RETURN
END SUBROUTINE verify_relia_bss_raw_qmapped_dressed
