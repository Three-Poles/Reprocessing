...
function(ensembleData, trainingDays, consecutive = FALSE, dates = NULL,
         control = controlMOSgev0(), warmStart = FALSE,
         exchangeable = NULL)
{
...
## determine the modeling dates
 ...
 s <- array( NA, c(1, nDates))
 dimnames(s) <- list(c("s"), dates)
 ...
 for(i in seq(along = juliandates)) {
    ...
    if(!consecutive){
      ...
      if (j != l) {
        ...
        fit <- fitMOSgev0(ensembleData[D,], control = control, exchangeable = exchangeable)
    ...
    if (warmStart) {
      if (is.null(exchangeable)){
        control$start <- list(a = fit$a, B = fit$B, s = fit$s, c = fit$c, d = fit$d, q = fit$q)
      }else {
        control$start <- list(a = fit$a, B = aggregate(fit$B, by=list(exchangeable),mean)[,2], s = fit$s, c = fit$c, d = fit$d, q = fit$q)
      }
    }
     cat("\n")
    print(round(c(fit$a, fit$B, fit$s),2))
