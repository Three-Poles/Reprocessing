...
function(ensembleData, trainingDays, consecutive = FALSE, dates = NULL,
         control = controlMOStruncnormal(), warmStart = FALSE,
         exchangeable = NULL)
{
...
## determine the modeling dates
 ...
 for(i in seq(along = juliandates)) {
    ...
    if(!consecutive){
      ...
      if (j != l) {
        ...
        fit <- fitMOStruncnormal(ensembleData[D,], control = control, exchangeable = exchangeable)
 ...
 structure(list(training = c(days = trainingDays, lag = lag,
		...
		a = a, B = B, c = c, d = d,
                ...
                call = match.call(), class = "ensembleMOStruncnormal")
}
